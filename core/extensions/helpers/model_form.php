    ...
    public static function create($model, $action = NULL)
    {
        ...
        foreach ($fields as $field) {
            ...
            if (in_array($field, $model->not_null)) {
                echo "<div class='form-group'><label for=\"$formId\" class=\"required\">$alias *</label>" . PHP_EOL;
            } else
                echo "<div class='form-group'><label for=\"$formId\">$alias</label>" . PHP_EOL;
            ...
            switch ($tipo) {
                ...
                case 'year': case 'day': case 'int unsigned': // Números
                    ...
                    if (strripos($field, '_id', -3)) {
                        ...
                        echo "<input id=\"$formId\" class='form-control' type=\"number\" name=\"$formName\" value=\"{$model->$field}\"><div>" . PHP_EOL;
                        break;
                    }

                case 'date': // Usar el js de datetime
                    echo "<input id=\"$formId\" class='form-control' type=\"date\" name=\"$formName\" value=\"{$model->$field}\"><div>" . PHP_EOL;
                    break;
                case 'datetime': case 'timestamp':
                    echo "<input id=\"$formId\" class='form-control' type=\"datetime\" name=\"$formName\" value=\"{$model->$field}\"><div>" . PHP_EOL;
                ...
                case 'enum': case 'set': case 'bool':
					...
					echo "<select id=\"$formId\" class=\"select\" form-control\" name=\"$formName\" >". PHP_EOL;
                ...
                case 'blob': case 'mediumblob': case 'longblob': // Usar textarea
                    echo "<textarea id=\"$formId\" class='form-control' name=\"$formName\">{$model->$field}</textarea><div>" . PHP_EOL;
                    break;

                default: //text,tinytext,varchar, char,etc se comprobara su tamaño
                    echo "<input id=\"$formId\" type=\"text\" name=\"$formName\" class='form-control' value=\"{$model->$field}\"><div>" . PHP_EOL;
                //break;
            }
        }
        //echo radio_field_tag("actuacion", array("U" => "una", "D" => "dos", "N" => "Nada"), "value: N");
        echo '<br><input type="submit" value="Enviar" class="btn btn-default" />' . PHP_EOL;
        echo '</form>' . PHP_EOL;
    }
